...
%% Import Lake points, DEM, FlowLength, and Flow Direction
    Lake = imread('N1_2015_GlacialLakes_minPt1km2_V2_pts_raster.tif');
        Lake = double(Lake);
        nrows = size(Lake,1);
        ncols = size(Lake,2);
    DEM = imread('River_N1_ASTER_Elev.tif');
        DEM = double(DEM);
    FlowDir = imread('River_N1_ASTER_FlowDir.tif');
