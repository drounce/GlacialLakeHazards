...
%% Import Lake points, DEM, FlowLength, and Flow Direction
    Lake = imread('N56_GlacialLakes_pts_V2_raster.tif');
        Lake = double(Lake);
        nrows = size(Lake,1);
        ncols = size(Lake,2);
    DEM = imread('River_N56_ASTER_Elev.tif');
        DEM = double(DEM);
    FlowDir = imread('River_N56_ASTER_FlowDir.tif');
