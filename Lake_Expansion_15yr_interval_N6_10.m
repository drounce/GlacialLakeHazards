...
Lake_Level = 5355; % Input lake level based on average from ASTER GDEM
Expansion_Rate = 0.058949911858; % km2 / 15 yr
    Expansion_Rate_Pixels = round(Expansion_Rate/(30*30*1/1000^2));
Lake_Number = 10; % Number for subset area (N1, N2, etc.), not all lakes
...
% Import tif files
Lake_2015 = imread('N6_2015_GlacialLakes_minPt1km2_raster_export.tif');
    Lake_2015 = double(Lake_2015);
Watershed = imread('N6_ASTER_Watershed.tif');
    Watershed = double(Watershed);
Ice = imread('N6_IceThickness_Glabtop2.tif');
    Ice = double(Ice);
DEM = imread('N6_ASTER_Elev_overdeepened.tif');
    DEM = double(DEM);
    nrows = size(DEM,1);
    ncols = size(DEM,2);

% Enter tif file data for writing tifs
Tif_Output_FileName = 'N6_LakeYear_2015_2045_N6_10';
Tif_Output_FilePath = 'C:\Dave_Rounce\MATLAB\Nepal_Hazards\Tiff_Files';
Tif_Input_FilePath = Tif_Output_FilePath;
Tif_Input_FileName = 'N6_ASTER_Elev_overdeepened.tif';
Tif_pixel_size = 30;
