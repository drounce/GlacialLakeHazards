...
%% Import DEM, Prone Areas, and Flow Accumulation
    Lake_Table = xlsread('N3_Lakes_AvalancheResults_Pt1km2_V4.xls');
        n_Lakes = size(Lake_Table,1);
    DEM = imread('N3_ASTER_Elev_V3.tif');
        DEM = double(DEM);
        nrows = size(DEM,1);
        ncols = size(DEM,2);
    FlowDir = imread('N3_ASTER_FlowDir_V3.tif');
        FlowDir = double(FlowDir);
            % Values of flow direction
                ...
                %  64 - Up
                % 128 - Up-Right
    ProneArea = imread('N3_nonGlacier_LandslideProne_V3.tif');
        ProneArea = double(ProneArea);
        ProneArea_Raw = ProneArea;
    Watershed = imread('N3_ASTER_Watershed_V3_raster.tif');
        Watershed = double(Watershed);
    Lake = imread('N3_2015_GlacialLakes_minPt1km2_V3_raster.tif');
        Lake = double(Lake);
    FileName_Prefix = 'N3_V4';
    pixel_size = 30;
