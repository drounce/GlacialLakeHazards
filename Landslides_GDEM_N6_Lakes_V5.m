...
%% Import DEM, Prone Areas, and Flow Accumulation
    Lake_Table = xlsread('N6_Lakes_AvalancheResults_Pt1km2_V4.xls');
        n_Lakes = size(Lake_Table,1);
    DEM = imread('N6_ASTER_Elev_V2.tif');
        DEM = double(DEM);
        nrows = size(DEM,1);
        ncols = size(DEM,2);
    FlowDir = imread('N6_ASTER_FlowDir_V2.tif');
