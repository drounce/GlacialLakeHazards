...
%% Import DEM, Prone Areas, and Flow Accumulation
    Lake_Table = xlsread('N2_Lakes_AvalancheResults_Pt1km2_V4.xls');
        n_Lakes = size(Lake_Table,1);
    DEM = imread('N2_Lakes_ASTER_Elev.tif');
        DEM = double(DEM);
        nrows = size(DEM,1);
        ncols = size(DEM,2);
    FlowDir = imread('N2_Lakes_ASTER_FlowDir.tif');
        FlowDir = double(FlowDir);
            % Values of flow direction
                ...
                %  64 - Up
                % 128 - Up-Right
    ProneArea = imread('N2_nonGlacier_LandslideProne.tif');
        ProneArea = double(ProneArea);
        ProneArea_Raw = ProneArea;
    Watershed = imread('N2_Lakes_ASTER_Watershed_V3.tif');
        Watershed = double(Watershed);
    Lake = imread('N2_2015_GlacialLakes_minPt1km2_V3_raster_export.tif');
        Lake = double(Lake);
    FileName_Prefix = 'N2_V4';
    pixel_size = 30;
