...
%% Import Lake points, DEM, FlowLength, and Flow Direction
    Lake = imread('N4_2015_GlacialLakes_minPt1km2_V3_pts_raster.tif');
        Lake = double(Lake);
        nrows = size(Lake,1);
        ncols = size(Lake,2);
    DEM = imread('River_N4_ASTER_Elev_V3.tif');
        DEM = double(DEM);
    FlowDir = imread('River_N4_ASTER_FlowDir_V3.tif');
